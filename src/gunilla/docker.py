...
def get_wordpress_container():
    ...
    return client.containers.get(workspace().config().wordpress_container_name())

def get_wordpress_container_ip():
    container = get_wordpress_container()
    return container.attrs['NetworkSettings']['Networks']['{}_default'.format(workspace().config().container_base_name())]['IPAddress']
